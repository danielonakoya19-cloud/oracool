...
- **Video generation now uses Hugging Face's Wan 2.2 models** (the three you linked — `Wan-AI/Wan2.2-T2V-A14B`, `Wan2.2-I2V-A14B`, `Wan2.2-TI2V-5B`):
  ...
  - **Without any token (free, works right away):** the server health-probes a pool of six live public Wan 2.2 I2V-14B Lightning spaces on Hugging Face (kekkonapoli, kingkladze, tmtanu, gum798, ivannm, Saravutw), tries them in health order, renders a first frame (CVRON/Agnes image engines) and animates it into a real .mp4 with the Wan 2.2 I2V-14B model. Unhealthy spaces are skipped automatically and each failure reason is stated honestly in chat. Note: free public spaces can degrade at times (2026-09-21: an HF-side outage rejected new jobs on every Wan 2.2 space for a while); the cascade re-probes on every request and picks the engine up the moment one is healthy again — no redeploy needed.
...
## Patch 23 — Device-grade OraCool: camera, voice in videos, full mailbox access, durable media

- **Build marker** `/api/health` → `patch23-device`.
- **📷 Camera ("take my picture" no longer refused):** when you ask for a picture/photo/selfie, the chat fires a tappable **📷 Take photo** capture card — tap it, allow the camera permission (front camera on phones), and the photo appears right in the chat: **view, ⬇️ Download, or ➕ Attach to your message** so OraCool can analyze it with you. The AI can never capture a photo by itself (browsers only allow camera access from a real user tap) — so it now always tells you to tap the card instead of refusing, and it can never claim a photo was taken without your tap. Works on phones and desktop; a clear recovery hint is shown if the permission is blocked.
- **🎬 Videos with voice:** "generate a video … with voice/sound" now delivers a clip that actually carries a voice — the Wan 2.2 picture is generated, a **narrated voice track (free neural TTS, Nigerian-English voice)** is generated, and both are **muxed into one .mp4** (static ffmpeg ships inside the pip package — no build-command change). With HiAPI credits the cascade still prefers **Veo 3.1's native synchronized audio** (real speech + ambient sound) and says so in chat. If a free engine was briefly down, the reply is honest about what the clip carries.
- **📧 FULL access to OraCool's mailbox** (admin-only, owner-granted): `oracoolai19@gmail.com` is OraCool's own address. In chat: **"check/read/show OraCool's inbox"** reads the latest messages **including body snippets** (no more unread-subjects-only), and **"send from oracool mail to <address>: <text>"** sends from that address (audited). One-time setup: **Admin → OraCool-owned accounts → connect the OraCool Gmail with a Google app password** — full access starts immediately after that one connect.
- **💾 Your creations survive refresh AND redeploys:** Render's disk is wiped on every deploy — that's why old videos/images 404'd after an update. Now every image/video you create is **uploaded to Supabase Storage** (public, permanent URL) in addition to the local copy; the chat and the media gallery always use the durable URL. Old media made before this patch was on the wiped disk and is gone; everything from this deploy forward persists. Also fixed: clips generated on the server (`/generated/`) were never served — they now are.
- **New deps** (picked up by the existing Render build — no dashboard change): `edge-tts` (free TTS), `imageio-ffmpeg` (static ffmpeg binary in the pip wheel).
